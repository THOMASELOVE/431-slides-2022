...
- A close look at the `dm431` data.

::: aside
Version `r Sys.time()`
:::

### Today's R Packages {.smaller}
...
library(janitor)
library(kableExtra) # for the kbl() function to neaten table output
...
- Use `{r, message = FALSE}` in the code chunk header to suppress messages about conflicts between R packages. 
...
- This is a sample of 431 people from a larger pool of people with diabetes which we'll study later this term.
- Note the use of `read_csv` instead of `read.csv` here.
    ...
    - Can also supress message with `{r, message = FALSE}` in the code chunk header.
...
## `dm431` variable names and specifications
...
- `gg_miss_var()` as shown on next screen, although that will throw a warning you can suppress by using `{r, warning = FALSE}` in the chunk header.
...
mosaic::favstats(~ age, data = dm431) |> kbl(digits = 2)
```
